...
export default function SimpleCamera({ onClose, onVideoRecorded, onPhotoTaken }: SimpleCameraProps) {
  ...
  if (recordedVideo) {
    return (
      <div className="fixed inset-0 bg-black z-50" style={{ width: '100vw', height: '100vh', overflow: 'hidden' }}>
        <video
          ref={previewVideoRef}
        ...
        <button
          ...
          className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-black/50 backdrop-blur-sm p-4 rounded-full z-50"
        >
          {isPlaying ? (
        ...
        <div className="absolute bottom-8 left-0 right-0 flex justify-between items-center px-8 z-50">
          <button
            onClick={retakeVideo}
            className="bg-white/20 backdrop-blur-sm p-4 rounded-full border-2 border-white/20 hover:bg-white/30 transition-all"
          >
          ...
          <button
            ...
            className="bg-gradient-to-r from-amber-500 to-amber-600 px-8 py-4 rounded-full flex items-center gap-2 shadow-lg hover:scale-105 transition-all"
          >
    ...
    <div className="fixed inset-0 bg-black z-50" style={{ width: '100vw', height: '100vh', overflow: 'hidden' }}>
      ...
      <div className="absolute top-0 left-0 right-0 bg-gradient-to-b from-black/85 via-amber-950/30 to-transparent z-50 pb-6">
        <div className="flex items-center justify-between px-4 py-4">
          <button
            onClick={onClose}
            className="p-3 rounded-full bg-black/50 backdrop-blur-sm border border-amber-900/30 hover:bg-black/70 transition-all"
          >
          ...
          <button
            ...
            className="p-4 rounded-full bg-black/50 backdrop-blur-sm border-2 border-white/20 hover:bg-black/70 transition-all disabled:opacity-50 shadow-lg shadow-black/50"
          >
        ...
        {mode === 'VIDEO' && !isRecording && (
          <div className="flex justify-center gap-4 mt-4">
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl backdrop-blur-sm transition-all ${
                videoType === 'littles'
                  ? 'bg-white/20 border-2 border-white text-white shadow-lg shadow-white/20'
                  : 'bg-black/20 border border-gray-600 text-gray-400'
              }`}
            >
              <Sparkles className="w-5 h-5" />
              <span className="font-medium">Littles</span>
            </button>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-6 py-3 rounded-xl backdrop-blur-sm transition-all ${
                videoType === 'length'
                  ? 'bg-green-300/20 border-2 border-green-300 text-green-300 shadow-lg shadow-green-300/20'
                  : 'bg-black/20 border border-gray-600 text-gray-400'
              }`}
            >
      ...
      {isRecording && (
        <div className="absolute top-24 left-0 right-0 flex justify-center z-40">
          <div className="bg-gradient-to-r from-red-500/90 to-red-600/90 backdrop-blur-sm px-6 py-2 rounded-full flex items-center gap-3 shadow-lg">
            ...
            <span className="text-white font-semibold text-lg tracking-wider antialiased">
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 via-amber-950/20 to-transparent pb-8 pt-12 z-40">
        <div className="flex justify-center items-center">
          {mode === 'VIDEO' ? (
            ...
            >
              <div className={`w-20 h-20 rounded-full flex items-center justify-center transition-all hover:scale-105 ${
                isRecording 
        ...
        <div className="text-center mt-4">
          {!streamRef.current && !isLoadingCamera && !error && (
            <p className="text-amber-400 text-sm animate-pulse">Waiting for camera access...</p>
          )}
          {mode === 'VIDEO' && !isRecording && streamRef.current && (
            <p className="text-white/70 text-sm">
              {videoType === 'littles' ? 'Tap to record short video (15s-3min)' : 'Tap to record long video (3min+)'}
            </p>
          )}
          {mode === 'PHOTO' && streamRef.current && (
            <p className="text-white/70 text-sm">Tap to capture photo</p>
          )}
        </div>
      </div>
    </div>
